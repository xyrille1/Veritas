...
export default function GasTreasuryPage() {
  ...
  const chartData = (data?.gasSeries ?? []).map(
    (d: { date: string; mist: number }) => ({
      date: d.date.slice(5), // MM-DD
      sui: parseFloat(parseFloat(mistToSui(d.mist)).toFixed(6)),
    }),
  );

  const balanceSui = data?.totalMist
    ? parseFloat(mistToSui(Number(data.totalMist)))
    : 0;
  ...
  return (
    <div className="space-y-6 py-8">
      <SectionHeader
        title="Gas Treasury"
        subtitle="Sponsor wallet balance and daily gas expenditure."
      />
        ...
        <>
          ...
          <div className="bg-[#112040] border border-white/5 rounded-[12px] p-6">
            <h2 className="text-white font-semibold mb-1">
              Daily Gas Expenditure (last 30 days)
            </h2>
            <p className="text-[#94A3B8] text-xs mb-6">
              Values in SUI (MIST converted)
            </p>
