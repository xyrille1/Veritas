  ...
  return (
    <div className="space-y-6 py-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <SectionHeader title="Audit Log" subtitle="Immutable record of all platform actions." />
        <select
          value={action}
      ...
      <div className="bg-[#112040] border border-white/5 rounded-[12px] p-6 overflow-x-auto">
        {isLoading ? (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {data.entries.map((e: { _id: string; createdAt: string; actorDisplay: string; actorAddress: string; action: string; resourceType: string; resourceId: string; txDigest?: string }) => (
                <tr key={e._id} className="border-b border-white/5 hover:bg-white/3">
                  <td className="py-2 px-2 text-[#94A3B8]">{relativeTime(e.createdAt)}</td>
                  <td className="py-2 px-2">
                    <div className="text-white">{e.actorDisplay}</div>
                    <div className="text-[#94A3B8] font-mono">{truncateAddress(e.actorAddress)}</div>
                  </td>
                  <td className="py-2 px-2">
                    <code className="text-[#2563EB] bg-[#2563EB]/10 px-1.5 py-0.5 rounded">{e.action}</code>
                  </td>
                  <td className="py-2 px-2 text-[#94A3B8]">
                    {e.resourceType}: <code className="font-mono text-white">{truncateAddress(e.resourceId)}</code>
                  </td>
                  <td className="py-2 px-2">
                    {e.txDigest && (
                      <a href={suiExplorerTxUrl(e.txDigest)} target="_blank" rel="noreferrer"
          ...
          <div className="flex justify-between mt-4 text-sm text-[#94A3B8]">
            ...
            <div className="flex gap-2">
              <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
                className="px-3 py-1 border border-white/10 rounded disabled:opacity-40">← Prev</button>
              <button onClick={() => setPage(p => p + 1)}
                className="px-3 py-1 border border-white/10 rounded">Next →</button>
            </div>
          </div>
        )}
      </div>
